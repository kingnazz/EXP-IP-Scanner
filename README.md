...
| Updates | Download the next ZIP | Quiet startup check + Check now |
...
- **The update check**, in the installed edition only, asks GitHub Releases
  which stable version is current once shortly after launch and whenever you
  press "Check now". The startup check is quiet and only surfaces an Update
  available badge when there is something newer. Signed releases can then be
  installed and relaunched in place. The portable edition does not contain the
  updater at all.
